  ...
  let pAequorFactory = (specimenNum, dna) => {
    return {
      ...
      mutate() {                                                                      // method that changes 1 random base
        ...
        while (randomBase === dna[index]) {                                               // loop until new random base is diff from the old base
        ...
        dna.splice(index, 1, randomBase);                                 // replacing the base with a random one
        console.log(`New dna strand: ${this.dna}`)
      },
      compareDNA(pAequor) {                                        // method to compare current organism with the previous one
        let counter = 0;
        for (let i = 0; i < this.dna.length; i++) {
          if (i == 0) {                                               // show the bases of the two organisms just one time each
            ...
            console.log(`Specimen${pAequor.specimenNum} bases: ${pAequor.dna}`);
          }
          if (this.dna[i] === pAequor.dna[i]) {                                      // if the two bases are equal
              console.log(this.dna[i] + " and  " + pAequor.dna[i] + " are equal");   // print message
        ...
        return `Specimen ${this.specimenNum} and Specimen ${pAequor.specimenNum} are ${basesInCommon}% compatible`;
      },
